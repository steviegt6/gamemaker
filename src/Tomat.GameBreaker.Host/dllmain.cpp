...
DWORD thread_main(LPVOID)
{
    ...
    // short* pCwd, short* pDllPath, int major, int minor, int patch
    native_entry native_entry;
    if (!init_dotnet(json, cwd, managed_host_dir, &native_entry))
    {
        MessageBox(nullptr, L"Failed to initialize .NET, cancelling injection.", L"Tomat.GameBreaker.Host", MB_OK | MB_ICONERROR);
        return 0;
    }

    native_entry(cwd.c_str(), dll_path.c_str(), MAJOR, MINOR, PATCH);
