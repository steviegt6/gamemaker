...
public abstract class LogWriter : ILogWriter {
    ...
    public static ILogWriter FromMany(params ILogWriter[] writers) {
...
public sealed class FileLogWriter : StringLogWriter {
    ...
    public FileLogWriter(string path, bool shared) {
        try {
            ...
            if (Path.GetDirectoryName(path) is { } dir && !string.IsNullOrWhiteSpace(dir))
...
internal sealed class NullLogWriter : LogWriter {
    public override void Write(ILogEntry entry) { }
}

internal sealed class ManyLogWriter : LogWriter {
